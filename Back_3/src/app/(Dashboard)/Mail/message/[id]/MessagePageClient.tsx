...
import { Paperclip, Download, ArrowLeft } from "lucide-react";
import { MessageByID, MessageFilesByID } from "@/Lib/ApiServiceMail";
...
export default function MessagePageClient({ id }: Props) {
    ...
    useEffect(() => {
        ...
        const run = async () => {
            ...
            try {
                const [resMsg, resFiles] = await Promise.all([MessageByID(mid), MessageFilesByID(mid)]);
                ...
                const parsedMsg: Msg = {
                    ...
                    Payam: String(row.Payam ?? ""), // ✅ همان HTML
    ...
    const fileUrl = (fileName: string) => `/MessageUploads/${encodeURIComponent(fileName)}`;
                    ...
                    {files.length === 0 ? (
                        ...
                        <div className="space-y-1.5">
                            {files.map((f, idx) => (
                                ...
                                >
                                    <div className="min-w-0 flex-1">
                                        <div className="truncate text-sm text-slate-800">{f.CaptionName}</div>
                                        {!!f.MMType && <div className="mt-0.5 truncate text-[11px] text-slate-500">{f.MMType}</div>}
                                    </div>

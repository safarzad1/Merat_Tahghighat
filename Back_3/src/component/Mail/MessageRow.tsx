    ...
    return (
        <Link
            ...
            className={[
                ...
                msg.isRead ? "opacity-90" : "ring-1 ring-slate-200",
                "hover:bg-slate-50",
            ].join(" ")}
        >
            ...
            <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-2">
                    <div className="truncate text-sm font-semibold">
                        ...
                        {!msg.isRead && (
                            <span className="mr-2 rounded-full bg-slate-900 px-2 py-0.5 text-[10px] text-white">
                                جدید
                            </span>
                        )}
                ...
                <div className="mt-1 truncate text-sm text-slate-900">
                    {msg.subject}
                    {msg.attachmentsCount > 0 && <span className="mr-2 text-slate-400">📎</span>}
                </div>

                <div className="mt-1 line-clamp-1 text-xs text-slate-500">{msg.preview}</div>
            </div>
        </Link>
    );
}

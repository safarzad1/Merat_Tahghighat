...
import { Update_ErjaParvandeh } from "@/Lib/ApiService";
...
const ErjaHozeh = ({ erjaParentId, onChangeCountErjaHozeh }: ErjaHozehProps) => {
  ...
  const ResultBargashToHozeh = async (erjaid: number, tozihat: string) => {
    const result = await Update_ErjaParvandeh(erjaid, false, 3, tozihat, user.UserId);
    if (result.state == 401) {
      router.push("/Login");
    }
    else {
      setModalBargashtParvandeh(false);
      await loadData();
    }
  };
              ...
              <button
                ...
                onClick={async () => {
                  if (tozihat.length > 10) {
                    ...
                    await ResultBargashToHozeh(idErja, tozihat);
                  }
                  else {

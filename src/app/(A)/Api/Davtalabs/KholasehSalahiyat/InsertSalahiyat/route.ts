import { NextRequest, NextResponse } from "next/server";
import sql from "mssql";

import { getConnection } from "@/Lib/db";
import { verifyToken } from "@/Lib/auth";

export async function POST(req: NextRequest) {
    const token = req.cookies.get("token")?.value;
    if (!token) {
        return NextResponse.json(
            { status: 401, error: "Unauthorized: No token" },
            { status: 401 }
        );
    }
    const body = await req.json();
    const { shomarehParvandeh, codeEntekhabat, natije, tozihat, typeUser, recordState, createUserId
    } = body;

    try {
        verifyToken(token);
    } catch {
        return NextResponse.json(
            { status: 401, error: "Unauthorized: Invalid or expired token" },
            { status: 401 }
        );
    }

    try {
        const pool = await getConnection();
        const result = await pool
            .request()
            .input("ShomarehParvandeh", sql.BigInt, shomarehParvandeh)
            .input("CodeEntekhabat", sql.BigInt, codeEntekhabat)
            .input("Natije", sql.Int, natije)
            .input("Tozihat", sql.NVarChar(3500), tozihat)
            .input("TypeUser", sql.Int, typeUser)
            .input("RecordState", sql.Int, recordState)
            .input("CreateUserId", sql.BigInt, createUserId)
            .execute("Davtalab.SP_InsertAshkhasSalahiyat");

        return NextResponse.json(
            { status: 200, data: result.recordset }
        );

    } catch (err) {
        return NextResponse.json(
            { error: err instanceof Error ? err.message : "Unknown error" },
            { status: 500 }
        );
    }
}

...
export async function POST(req: NextRequest) {
    ...
    const { erjaId, fileName, fileExtend, fileSize, userid } = body;
    ...
    try {
        ...
        const result = await pool
            .request()
            .input("IDErja", sql.BigInt, erjaId)
            .input("FileName", sql.NVarChar(200), fileName)
            .input("FileExtend", sql.NVarChar(200), fileExtend)
            .input("FileSize", sql.NVarChar, fileSize)
            .input("UserId", sql.Int, userid)
            .execute("Tahghighat.SP_Insert_JambandiOstan_Peyvast");

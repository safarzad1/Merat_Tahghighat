...
import { GetAmarNemodarHamkaran } from '@/Lib/ApiServiseHamkari';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, Cell } from 'recharts';
import { useRouter } from "next/navigation";

interface DataItem {
    NoeHamkari_NameFarsi: string;  // تغییر داده شده
...
export default function FileStatusChart({ mahal }: FileStatusChartProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const response: ApiResponse = await GetAmarNemodarHamkaran(mahal);
    ...
    const CustomTooltip = ({ active, payload, label }: any) => {
        if (active && payload && payload.length) {
            const index = chartData.findIndex(item => item.NoeHamkari_NameFarsi === label);
                ...
                <div className="bg-white p-4 shadow-lg rounded-lg border border-gray-200">
                    ...
                    <div className="flex items-center">
                        ...
                        <p className="text-gray-600">
                            تعداد: <span className="font-bold text-blue-600">
                ...
                <div className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                            ...
                            />
                            <XAxis
                                dataKey="NoeHamkari_NameFarsi"  // تغییر داده شده
                                axisLine={false}
                                tickLine={false}
                            ...
                            >
                                {chartData.map((entry, index) => (
                                    <Cell
                                        ...
                                        strokeWidth={index === chartData.length - 1 ? 0 : 1}
                                        stroke="#fff"
                                    />
                                ))}
                            </Bar>
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-3">
                        <div className="flex items-center">
                            ...
                            <span className="text-sm text-gray-600">
                                مجموع: {chartData.reduce((sum, item) => sum + item.CountParvandeh, 0).toLocaleString('fa-IR')} نفر
                            </span>
                        </div>
                        ...
                        {chartData.map((item, index) => (
                            <div key={index} className="bg-gray-50 p-3 rounded-md">
                                <div className="flex items-center mb-1">
                                    ...
                                    ></div>
                                    <span className="text-sm text-gray-600 truncate">{item.NoeHamkari_NameFarsi}</span>
                                </div>
                                <div className="text-base font-bold text-gray-800">
                                    {item.CountParvandeh.toLocaleString('fa-IR')}
                                </div>
                            </div>
                        ))}
                    </div>
